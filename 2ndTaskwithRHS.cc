...
const double pi= 3.141592653589793238463;
  ...
  double RightHandSide<dim>::RHSvalue (const Point<dim> &p,const unsigned int component = 0)
  {
    ...
    for (unsigned int i=0; i<dim; ++i)
    {
      ...
      return_value += (8.0 * exp((-4.0 * radial_distance)/ r_c) - exp((-radial_distance)/r_c))/(std::pow(r_c,1.5) * std::pow(pi, 1.5))  ;
    return return_value;
  }
  ...
  {
    for (unsigned int cycle=0; cycle<3; ++cycle) // first mesh size 4^2 = 16*16*16 and then 2 refinements
      {
        ...
        if (cycle == 0)
          {
            ...
            triangulation.refine_global (2);  //first mesh size 4^2 = 16*16*16
          }
        else
          refine_grid ();
